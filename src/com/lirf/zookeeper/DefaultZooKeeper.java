...
public class DefaultZooKeeper {
    ...
    private void syncOperations(ZooKeeper zk, Watcher watcher) throws Exception {
        ...
        if (stat == null) {
            ...
            zk.create("/hifreud", "Hi Freud".getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
        }
        ...
        if (stat != null) {
            if (zk.exists("/hifreud/hi", watcher) == null) {
                zk.create("/hifreud/hi", "Hi Freud - hi".getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
            }
        }
    ...
    private void asyncOperations(ZooKeeper zk) throws Exception {
        ...
        // 异步创建节点
        if (stat == null) {
            zk.create("/asynchifreud", "hifreud".getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT,
    ...
    private void authOperations(Watcher watcher) throws Exception {
        ...
        if (stat == null) {
            // 带权限创建节点
            zk1.create(authPath1, "Hi freud".getBytes(), ZooDefs.Ids.CREATOR_ALL_ACL, CreateMode.PERSISTENT);
        }
        ...
        if (stat == null) {
            // 带权限创建子节点
            zk1.create(authPath2, "Hi freud 2".getBytes(), ZooDefs.Ids.CREATOR_ALL_ACL, CreateMode.PERSISTENT);
        }
